...
** minor settings
*** blinking point
    Incosistent naming... cursor is called point in Emacs however you
    turn it off with (blinking-cursor-mode -1). I just noticed that
    the cursor stops blinking after a while. It starts blinking again
    when you move the cursor. I guess this is also configurable in
    Emacs. It is. Set blink-cursor-blinks to 0 or a negative value to
    stay blinking.
#+begin_src emacs-lisp :tangle yes
  ;; (blink-cursor-mode -1)
