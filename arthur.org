...
*** DONE move non-related code comments out of code blocks
*** DONE setup org fast access to TODO states
*** DONE clean up the packages section
*** DONE treemacs?
    treemacs-persp, treemacs-projectile
*** DONE have a look at dired integration with projectile
    Maybe there is no need for treemacs. update: C-c p m D to open
    project files with dired mode
*** DONE setup dired-hide-dotfiles
